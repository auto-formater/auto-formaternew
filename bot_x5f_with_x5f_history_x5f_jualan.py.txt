...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_profil":
        ...
        if row:
            ...
            # NOTIFIKASI KE SESAMA ADMIN SAJA (user biasa tidak dapat notif)
            if is_admin_seller:
                notif_text = (
                    f"🔔 **DATA TERJUAL - NOTIF ADMIN**\n"
                    f"━━━━━━━━━━━━━━━━━━━\n"
                    f"📦 Kode: {kode} (ID {rid})\n"
                    f"📍 Lokasi: {kab}/{kec}/{kel}\n"
                    f"👤 Pemilik awal: {owner_nama} (@{owner_username}) ID {owner_id}\n"
                    f"💰 Dijual oleh: {seller_name} (@{seller_username}) ID {seller_id}\n"
                    f"⏰ Waktu: {datetime.now().strftime('%d-%m-%Y %H:%M:%S')}\n"
                    f"━━━━━━━━━━━━━━━━━━━\n"
                    f"Data sudah dipindah ke HISTORY (terjual)"
                )
                for admin_id in all_admins:
                    if admin_id != seller_id:  # jangan kirim ke diri sendiri (sudah dapat notif di atas)
                        try:
