...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_profil":
        ...
        if row:
            ...
            # === FIX: DATA TERJUAL JANGAN DIKIRIM KE USER LAIN ===
            # Broadcast dimatikan total sesuai request: data yang sudah terjual tidak dikirim ke user
            # Jika tetap ingin notifikasi tanpa detail data, uncomment blok di bawah:
            # if is_admin_seller:
            ...
            #     conn2.close()
            #     await context.bot.send_message(chat_id=seller_id, text=f"✅ ID {rid} ({kode}) ditandai terjual. Tidak dibroadcast ke user.")
            pass
                except:
                    pass
        else:
