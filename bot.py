...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_profil":
        ...
        if not has_access(user_id):
            await query.edit_message_text(
                "🔒 **MENU TERKUNCI**\n\n"
                "Untuk melihat **HASIL FORMAT**, kamu harus berlangganan dulu.\n\n"
                "Pilih paket di bawah:",
                reply_markup=paket_menu_keyboard(),
                parse_mode="Markdown"
            )
        ...
        if not has_access(user_id):
            await query.edit_message_text(
                "🔒 **MENU TERKUNCI**\n\n"
                "Untuk melihat **FORMAT+AKUN**, kamu harus berlangganan dulu.\n\n"
                "Pilih paket di bawah:",
                reply_markup=paket_menu_keyboard(),
                parse_mode="Markdown"
            )
        ...
        if not has_access(user_id):
            await query.edit_message_text(
                "🔒 **MENU TERKUNCI**\n\n"
                "Untuk menggunakan **CARI DATA**, kamu harus berlangganan dulu.\n\n"
                "Pilih paket di bawah:",
                reply_markup=paket_menu_keyboard(),
                parse_mode="Markdown"
            )
    ...
    elif data == "data_saya_format":
        if not has_access(user_id):
            await query.edit_message_text(
                "🔒 **MENU TERKUNCI**\n\n"
                "Untuk menggunakan **DATA SAYA - FORMAT SAJA**, kamu harus berlangganan dulu.\n\n"
                "Pilih paket di bawah:",
                reply_markup=paket_menu_keyboard(),
                parse_mode="Markdown"
            )
            return
        context.user_data["mode"] = "data_saya_format"
        template, _, _, kode_pos, _ = get_setting(user_id)
        next_kode = get_next_code(user_id)
        await query.edit_message_text(
            f"📄 FORMAT SAJA - DATA SAYA\n\n"
            f"Kode aktif: {next_kode} (Posisi: {kode_pos.upper()})\n\n"
            f"Silahkan kirim data format saja\n"
            f"Kirim seperti ini (9 baris):\n"
            f"DEPOK\nCILODONG\nKALIBARU\n10.000.000\nPEREMPUAN 1992\n2019\n23****\n01-07-2022\nINDONESIA MERDEKA\n\n"
            f"Bisa kirim 30 sekaligus, pisah dengan baris kosong!\n"
            f"Otomatis masuk di menu HASIL DATA SAYA > HASIL FORMAT\n\n"
            f"Ketik data sekarang 👇",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ KEMBALI", callback_data="menu_data_saya")]])
        )
        return

    elif data == "data_saya_full":
        if not has_access(user_id):
            await query.edit_message_text(
                "🔒 **MENU TERKUNCI**\n\n"
                "Untuk menggunakan **DATA SAYA - FORMAT+AKUN**, kamu harus berlangganan dulu.\n\n"
                "Pilih paket di bawah:",
                reply_markup=paket_menu_keyboard(),
                parse_mode="Markdown"
            )
            return
        context.user_data["mode"] = "data_saya_full"
        template, _, _, kode_pos, _ = get_setting(user_id)
        next_kode = get_next_code(user_id)
        await query.edit_message_text(
            f"📑 FORMAT+AKUN - DATA SAYA\n\n"
            f"Kode aktif: {next_kode} (Posisi: {kode_pos.upper()})\n\n"
            f"Silahkan kirim data format+akun anda otomatis masuk di menu HASIL DATA SAYA di menu HASIL FORMAT+AKUN\n\n"
            f"Kirim seperti ini (13 baris):\n"
            f"DEPOK\nCILODONG\nKALIBARU\n10.000.000\nPEREMPUAN 1992\n2019\n23****\n01-07-2022\nINDONESIA MERDEKA\n320123...\n19000...\nLIA\n12-12-1990\n\n"
            f"Bisa kirim 30 sekaligus!\nKetik sekarang 👇",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ KEMBALI", callback_data="menu_data_saya")]])
        )
        return
        ...
        if not has_access(user_id):
            await query.edit_message_text(
                "🔒 **MENU TERKUNCI**\n\n"
                "Untuk melihat **HASIL DATA SAYA**, kamu harus berlangganan dulu.\n\n"
                "Pilih paket di bawah:",
                reply_markup=paket_menu_keyboard(),
                parse_mode="Markdown"
            )
        ...
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("SELECT COUNT(*) FROM data_saya WHERE user_id=? AND (status='ready' OR status IS NULL)", (user_id,))
        total_ready = c.fetchone()[0]
        c.execute("SELECT COUNT(*) FROM data_saya WHERE user_id=? AND tipe='format' AND (status='ready' OR status IS NULL)", (user_id,))
        total_format = c.fetchone()[0]
        c.execute("SELECT COUNT(*) FROM data_saya WHERE user_id=? AND tipe='full' AND (status='ready' OR status IS NULL)", (user_id,))
        total_full = c.fetchone()[0]
        # fallback if tipe null (old data) -> count all as format
        if total_format==0 and total_full==0 and total_ready>0:
            total_format = total_ready
        conn.close()
        if total_ready==0:
            await query.edit_message_text(
                "📊 HASIL DATA SAYA\n\nBelum ada data. Silahkan tambah di menu DATA SAYA dulu.\nKirim 30 data sekaligus juga bisa!\n\nData yang dikirim di DATA SAYA akan masuk ke:\n- HASIL FORMAT SAJA\n- HASIL FORMAT+AKUN",
                reply_markup=InlineKeyboardMarkup([
            ...
            return
        # Menu utama HASIL DATA SAYA - sesuai request: HASIL FORMAT SAJA & HASIL FORMAT+AKUN dari DATA SAYA
        await query.edit_message_text(
            f"📊 HASIL DATA SAYA\n\n"
            f"Total tersimpan: {total_ready} data\n"
            f"📄 FORMAT SAJA: {total_format} data\n"
            f"📑 FORMAT+AKUN: {total_full} data\n\n"
            f"Data yang dikirim dari DATA SAYA disimpan di sini:\n"
            f"- FORMAT SAJA → dari menu 1. FORMAT SAJA di DATA SAYA\n"
            f"- FORMAT+AKUN → dari menu 2. KIRIM FORMAT+AKUN di DATA SAYA\n\n"
            f"Pilih tampilan hasil:",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton(f"📄 1. HASIL FORMAT SAJA ({total_format})", callback_data="hasil_data_format")],
                [InlineKeyboardButton(f"📑 2. HASIL FORMAT+AKUN ({total_full})", callback_data="hasil_data_full")],
                [InlineKeyboardButton("⬅️ 3. KEMBALI", callback_data="back_main")]
            ])
        )
        return

    elif data == "hasil_data_format" or data.startswith("hasil_data_format_page_"):
        if not has_access(user_id):
            await query.edit_message_text(
                "🔒 **MENU TERKUNCI**\n\n"
                "Untuk melihat **HASIL FORMAT SAJA**, kamu harus berlangganan dulu.\n\n"
                "Pilih paket di bawah:",
                reply_markup=paket_menu_keyboard(),
                parse_mode="Markdown"
            )
            return
        # Pagination 50 data per halaman untuk HASIL FORMAT SAJA dari DATA SAYA
        page = 1
        if "page_" in data:
            try:
        ...
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        # Hitung total FORMAT SAJA (tipe format) - fallback jika tipe null hitung semua ready
        c.execute("SELECT COUNT(*) FROM data_saya WHERE user_id=? AND tipe='format' AND (status='ready' OR status IS NULL)", (user_id,))
        total_tipe = c.fetchone()[0]
        if total_tipe==0:
            c.execute("SELECT COUNT(*) FROM data_saya WHERE user_id=? AND (status='ready' OR status IS NULL)", (user_id,))
            total = c.fetchone()[0]
            c.execute("SELECT id, display_text, raw_text, kode FROM data_saya WHERE user_id=? AND (status='ready' OR status IS NULL) ORDER BY id ASC LIMIT ? OFFSET ?", (user_id, per_page, offset))
        else:
            total = total_tipe
            c.execute("SELECT id, display_text, raw_text, kode FROM data_saya WHERE user_id=? AND tipe='format' AND (status='ready' OR status IS NULL) ORDER BY id ASC LIMIT ? OFFSET ?", (user_id, per_page, offset))
        rows = c.fetchall()
        conn.close()
        if not rows:
            if page==1:
                await query.edit_message_text("📄 HASIL FORMAT SAJA - Belum ada data dari DATA SAYA. Kirim dulu di DATA SAYA > FORMAT SAJA", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📂 KIRIM DATA", callback_data="data_saya_format")],[InlineKeyboardButton("⬅️ KEMBALI", callback_data="menu_hasil_data_saya")]]))
            else:
                await query.edit_message_text(f"📄 Halaman {page} kosong", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ KEMBALI", callback_data="menu_hasil_data_saya")]]))
            return
        total_pages = (total + per_page -1)//per_page
        start_num = offset+1
        end_num = min(offset+len(rows), total)
        ...
        nav_buttons = []
        if page>1:
            nav_buttons.append(InlineKeyboardButton("⬅️ SEBELUMNYA", callback_data=f"hasil_data_format_page_{page-1}"))
        if page < total_pages:
            nav_buttons.append(InlineKeyboardButton("➡️ BERIKUTNYA", callback_data=f"hasil_data_format_page_{page+1}"))
        kb_top = [[InlineKeyboardButton("🔍 CARI DATA", callback_data="cari_data_saya")]]
        if nav_buttons:
            kb_top.append(nav_buttons)
        kb_top.append([InlineKeyboardButton("⬅️ KEMBALI", callback_data="menu_hasil_data_saya")])
        await query.edit_message_text(
            f"📄 HASIL FORMAT SAJA - Halaman {page}/{total_pages}\n"
            f"Total dari DATA SAYA: {total} data | Tampil: {start_num}-{end_num} (50 per halaman)\n"
            f"Ini menyimpan data yang dikirim dari DATA SAYA > FORMAT SAJA",
            reply_markup=InlineKeyboardMarkup(kb_top)
        )
        ...
        bottom_nav = []
        if page>1:
            bottom_nav.append(InlineKeyboardButton("⬅️ SEBELUMNYA", callback_data=f"hasil_data_format_page_{page-1}"))
        if page < total_pages:
            bottom_nav.append(InlineKeyboardButton("➡️ BERIKUTNYA", callback_data=f"hasil_data_format_page_{page+1}"))
        kb_bottom=[]
        if bottom_nav:
            kb_bottom.append(bottom_nav)
        kb_bottom.append([InlineKeyboardButton("🔍 CARI DATA", callback_data="cari_data_saya")])
        kb_bottom.append([InlineKeyboardButton("⬅️ KEMBALI", callback_data="menu_hasil_data_saya")])
        await context.bot.send_message(chat_id=user_id, text=f"📄 Halaman {page}/{total_pages} | {start_num}-{end_num} dari {total} - HASIL FORMAT SAJA (dari DATA SAYA)", reply_markup=InlineKeyboardMarkup(kb_bottom))
        return

    elif data == "hasil_data_full" or data.startswith("hasil_data_full_page_"):
        if not has_access(user_id):
            await query.edit_message_text(
                "🔒 **MENU TERKUNCI**\n\n"
                "Untuk melihat **HASIL FORMAT+AKUN**, kamu harus berlangganan dulu.\n\n"
                "Pilih paket di bawah:",
                reply_markup=paket_menu_keyboard(),
                parse_mode="Markdown"
            )
            return
        # Pagination 50 untuk HASIL FORMAT+AKUN dari DATA SAYA
        page = 1
        if "page_" in data:
            try:
                page = int(data.split("_")[-1])
        ...
        total_tipe = c.fetchone()[0]
        if total_tipe==0:
            # jika tipe belum diset, coba hitung yang punya nik_lengkap / full display
            c.execute("SELECT COUNT(*) FROM data_saya WHERE user_id=? AND (nik_lengkap!='' OR kpj_lengkap!='') AND (status='ready' OR status IS NULL)", (user_id,))
            total_alt = c.fetchone()[0]
            if total_alt>0:
                total = total_alt
                c.execute("SELECT id, display_text, raw_text, kode, kab, kec, kel, nik_lengkap, kpj_lengkap, nama_lengkap, tgl_lahir FROM data_saya WHERE user_id=? AND (nik_lengkap!='' OR kpj_lengkap!='') AND (status='ready' OR status IS NULL) ORDER BY id ASC LIMIT ? OFFSET ?", (user_id, per_page, offset))
            else:
                c.execute("SELECT COUNT(*) FROM data_saya WHERE user_id=? AND (status='ready' OR status IS NULL)", (user_id,))
                total = c.fetchone()[0]
                c.execute("SELECT id, display_text, raw_text, kode, kab, kec, kel, nik_lengkap, kpj_lengkap, nama_lengkap, tgl_lahir FROM data_saya WHERE user_id=? AND (status='ready' OR status IS NULL) ORDER BY id ASC LIMIT ? OFFSET ?", (user_id, per_page, offset))
        else:
            total = total_tipe
            c.execute("SELECT id, display_text, raw_text, kode, kab, kec, kel, nik_lengkap, kpj_lengkap, nama_lengkap, tgl_lahir FROM data_saya WHERE user_id=? AND tipe='full' AND (status='ready' OR status IS NULL) ORDER BY id ASC LIMIT ? OFFSET ?", (user_id, per_page, offset))
        rows = c.fetchall()
        conn.close()
        if not rows:
            if page==1:
                await query.edit_message_text("📑 HASIL FORMAT+AKUN - Belum ada data dari DATA SAYA. Kirim dulu di DATA SAYA > KIRIM FORMAT+AKUN", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📂 KIRIM DATA", callback_data="data_saya_full")],[InlineKeyboardButton("⬅️ KEMBALI", callback_data="menu_hasil_data_saya")]]))
            else:
                await query.edit_message_text(f"📑 Halaman {page} kosong", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ KEMBALI", callback_data="menu_hasil_data_saya")]]))
            return
        total_pages = (total + per_page -1)//per_page
        ...
        nav_buttons = []
        if page>1:
            nav_buttons.append(InlineKeyboardButton("⬅️ SEBELUMNYA", callback_data=f"hasil_data_full_page_{page-1}"))
        if page < total_pages:
            nav_buttons.append(InlineKeyboardButton("➡️ BERIKUTNYA", callback_data=f"hasil_data_full_page_{page+1}"))
        kb_top = [[InlineKeyboardButton("🔍 CARI DATA", callback_data="cari_data_saya")]]
        if nav_buttons:
            kb_top.append(nav_buttons)
        kb_top.append([InlineKeyboardButton("⬅️ KEMBALI", callback_data="menu_hasil_data_saya")])
        await query.edit_message_text(
            f"📑 HASIL FORMAT+AKUN - Halaman {page}/{total_pages}\n"
            f"Total dari DATA SAYA: {total} data | Tampil: {start_num}-{end_num} (50 per halaman)\n"
            f"Ini menyimpan data yang dikirim dari DATA SAYA > KIRIM FORMAT+AKUN (format + DATA LENGKAP)",
            reply_markup=InlineKeyboardMarkup(kb_top)
        )
        ...
        bottom_nav=[]
        if page>1:
            bottom_nav.append(InlineKeyboardButton("⬅️ SEBELUMNYA", callback_data=f"hasil_data_full_page_{page-1}"))
        if page < total_pages:
            bottom_nav.append(InlineKeyboardButton("➡️ BERIKUTNYA", callback_data=f"hasil_data_full_page_{page+1}"))
        kb_bottom=[]
        if bottom_nav:
            kb_bottom.append(bottom_nav)
        kb_bottom.append([InlineKeyboardButton("🔍 CARI DATA", callback_data="cari_data_saya")])
        kb_bottom.append([InlineKeyboardButton("⬅️ KEMBALI", callback_data="menu_hasil_data_saya")])
        await context.bot.send_message(chat_id=user_id, text=f"📑 Halaman {page}/{total_pages} | {start_num}-{end_num} dari {total} - HASIL FORMAT+AKUN (dari DATA SAYA)", reply_markup=InlineKeyboardMarkup(kb_bottom))
        return
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        ...
        if not has_access(user_id):
            await query.edit_message_text(
                "🔒 **MENU TERKUNCI**\n\n"
                "Untuk menggunakan **CARI DATA SAYA**, kamu harus berlangganan dulu.\n\n"
                "Pilih paket di bawah:",
                reply_markup=paket_menu_keyboard(),
                parse_mode="Markdown"
            )
        ...
        # Hapus permanen data_saya (baik dari status maupun dari deleted table)
        try:
            del_id = int(data.split("_")[-1])
        except:
            await query.edit_message_text("❌ ID tidak valid", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ KEMBALI", callback_data="menu_history_data_saya")]]))
            return
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        # Coba hapus dari data_saya (status terjual/terhapus) dan dari deleted table
        try:
            c.execute("DELETE FROM data_saya WHERE id=? AND user_id=?", (del_id, user_id))
        except:
            pass
        try:
            c.execute("DELETE FROM data_saya_deleted WHERE id=? AND user_id=?", (del_id, user_id))
            c.execute("DELETE FROM data_saya_deleted WHERE orig_id=? AND user_id=?", (del_id, user_id))
        except:
            pass
        conn.commit()
        conn.close()
        await query.edit_message_text(f"🗑️ ID {del_id} dihapus permanen.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🕘 HISTORY DATA SAYA", callback_data="menu_history_data_saya")],[InlineKeyboardButton("⬅️ KEMBALI", callback_data="back_main")]]))
        return

    elif data.startswith("pulihkan_data_"):
        try:
            rid = int(data.split("_")[-1])
        except:
            await query.edit_message_text("❌ ID tidak valid", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ KEMBALI", callback_data="menu_history_data_saya")]]))
        ...
        c = conn.cursor()
        try:
            c.execute("UPDATE data_saya SET status='ready' WHERE id=? AND user_id=?", (rid, user_id))
            conn.commit()
            await query.edit_message_text(f"♻️ ID {rid} berhasil dipulihkan ke HASIL DATA SAYA!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📊 HASIL DATA SAYA", callback_data="menu_hasil_data_saya")],[InlineKeyboardButton("🕘 HISTORY DATA SAYA", callback_data="menu_history_data_saya")],[InlineKeyboardButton("⬅️ KEMBALI", callback_data="back_main")]]))
        except Exception as e:
            await query.edit_message_text(f"❌ Gagal pulihkan: {e}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ KEMBALI", callback_data="menu_history_data_saya")]]))
        finally:
            conn.close()
        return

    elif data == "cari_history_data_jual":
        if not has_access(user_id):
            await query.edit_message_text(
                "🔒 **MENU TERKUNCI**\n\nUntuk CARI HISTORY JUAL harus berlangganan.\nPilih paket:",
                reply_markup=paket_menu_keyboard(),
                parse_mode="Markdown"
            )
            return
        context.user_data["mode"] = "cari_history_data_jual"
        await query.edit_message_text(
            "🔍 CARI HISTORY JUAL DATA SAYA\n\nKetik kata kunci (kode, nama, KAB, dll):",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ KEMBALI", callback_data="history_data_jual")]])
        )
        return

    elif data == "cari_history_data_hapus":
        if not has_access(user_id):
            await query.edit_message_text(
                "🔒 **MENU TERKUNCI**\n\nUntuk CARI HISTORY HAPUS harus berlangganan.\nPilih paket:",
                reply_markup=paket_menu_keyboard(),
                parse_mode="Markdown"
            )
            return
        context.user_data["mode"] = "cari_history_data_hapus"
        await query.edit_message_text(
            "🔍 CARI HISTORY HAPUS DATA SAYA\n\nKetik kata kunci (kode, nama, KAB, dll):",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ KEMBALI", callback_data="history_data_hapus")]])
        )
        return
        ...
        if not has_access(user_id):
            await query.edit_message_text(
                "🔒 **MENU TERKUNCI**\n\n"
                "Untuk melihat **HISTORY DATA SAYA**, kamu harus berlangganan dulu.\n\n"
                "Pilih paket di bawah:",
                reply_markup=paket_menu_keyboard(),
                parse_mode="Markdown"
            )
        ...
        if not has_access(user_id):
            await query.edit_message_text(
                "🔒 **MENU TERKUNCI**\n\n"
                "Untuk melihat **HISTORY JUAL DATA SAYA**, kamu harus berlangganan dulu.\n\n"
                "Pilih paket di bawah:",
                reply_markup=paket_menu_keyboard(),
                parse_mode="Markdown"
            )
        ...
        if not has_access(user_id):
            await query.edit_message_text(
                "🔒 **MENU TERKUNCI**\n\n"
                "Untuk melihat **HISTORY HAPUS DATA SAYA**, kamu harus berlangganan dulu.\n\n"
                "Pilih paket di bawah:",
                reply_markup=paket_menu_keyboard(),
                parse_mode="Markdown"
            )
