...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text("👋 Selamat datang! Pilih menu di bawah:", reply_markup=main_menu())
    except Exception as e:
    ...
    try:
        ...
        if data == "profil":
            try:
                conn = get_conn()
                c = conn.cursor()
                c.execute("SELECT username, first_name, join_date FROM users WHERE user_id=?", (user_id,))
                row = c.fetchone()
                conn.close()
                username = (row[0] if row and row[0] else "-")
                first_name = (row[1] if row and row[1] else query.from_user.first_name or "-")
                join_date = (row[2] if row and row[2] else "-")
                await query.edit_message_text(f"👤 PROFIL\n\nID: {user_id}\nNama: {first_name}\nUsername: @{username if username != '-' else '-'}\nBergabung: {join_date}", reply_markup=main_menu())
            except Exception as e:
