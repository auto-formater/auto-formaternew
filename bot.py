...
print(f"✅ BOT READY | ADMIN: {ADMIN_IDS}")
...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS data_saya (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, kode TEXT, kab TEXT, kec TEXT, kel TEXT, saldo TEXT, kelamin TEXT, kpj TEXT, sensor TEXT, it TEXT, pt TEXT, nik_lengkap TEXT, kpj_lengkap TEXT, nama_lengkap TEXT, tgl_lahir TEXT, raw_text TEXT, display_text TEXT, created_at TEXT)""")
    c.execute("""CREATE TABLE IF NOT EXISTS paket_user (user_id INTEGER PRIMARY KEY, paket_name TEXT, added_by INTEGER, created_at TEXT, expired_at TEXT)""")
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_profil":
        ...
        if is_admin and "ADMIN" not in str(expired_db).upper():
            expired_display = f"{expired_db} ADMIN" if expired_db != "UNLIMITED" else "UNLIMITED ADMIN"
        else:
            expired_display = expired_db
    ...
    elif data == "menu_hasil_format" or data.startswith("hf_page_") or data == "tampilkan_format" or data.startswith("hf_next_") or data.startswith("hf_prev_"):
        # Pagination: 50 data per halaman
        page = 0
        if data.startswith("hf_page_"):
            try:
                page = int(data.split("_")[-1])
            except:
                page = 0
        elif data.startswith("hf_next_"):
            try:
                page = int(data.split("_")[-1]) + 1
            except:
                page = 0
        elif data.startswith("hf_prev_"):
            try:
                page = int(data.split("_")[-1]) - 1
                if page < 0:
                    page = 0
        ...
        if total == 0:
            ...
            c2c = c2.cursor()
            c2c.execute("SELECT COUNT(*) FROM hasil_format WHERE user_id=?", (user_id,))
            total_all = c2c.fetchone()[0]
            c2.close()
            if total_all > 0:
        ...
        await query.edit_message_text(
            ...
            f"Total: {total} data | Menampilkan {start_num}-{end_num}\n"
            f"Urutan: {rows[0][1]} → {rows[-1][1] if rows else '-'}",
            reply_markup=InlineKeyboardMarkup([
        ...
        nav_row = []
        if page > 0:
            nav_row.append(InlineKeyboardButton("⬅️ SEBELUMNYA", callback_data=f"hf_prev_{page}"))
        if page < total_pages - 1:
            nav_row.append(InlineKeyboardButton("➡️ BERIKUTNYA", callback_data=f"hf_next_{page}"))
        
        nav_kb = []
        if nav_row:
            nav_kb.append(nav_row)
        nav_kb.append([InlineKeyboardButton("🔍 CARI DATA", callback_data="cari_format")])
        nav_kb.append([InlineKeyboardButton("⬅️ KEMBALI KE MENU", callback_data="back_main")])
        
        await context.bot.send_message(chat_id=user_id, text=f"📄 Halaman {page+1}/{total_pages} - {total} data", reply_markup=InlineKeyboardMarkup(nav_kb))

    elif data == "tampilkan_full" or data == "menu_hasil_full" or data.startswith("hff_page_") or data.startswith("hff_next_") or data.startswith("hff_prev_"):
        page = 0
        if data.startswith("hff_page_"):
            try:
                page = int(data.split("_")[-1])
            except:
                page = 0
        elif data.startswith("hff_next_"):
            try:
                page = int(data.split("_")[-1]) + 1
            except:
                page = 0
        elif data.startswith("hff_prev_"):
            try:
                page = int(data.split("_")[-1]) - 1
                if page < 0:
                    page = 0
        ...
        await query.edit_message_text(
            f"📑 **HASIL FORMAT+AKUN** - Halaman {page+1}/{total_pages}\n"
            f"Total: {total} data | Menampilkan {start_num}-{end_num}\n"
            f"Urutan: {rows[0][1]} → {rows[-1][1] if rows else '-'}",
            reply_markup=InlineKeyboardMarkup([
        ...
        nav_row = []
        if page > 0:
            nav_row.append(InlineKeyboardButton("⬅️ SEBELUMNYA", callback_data=f"hff_prev_{page}"))
        if page < total_pages - 1:
            nav_row.append(InlineKeyboardButton("➡️ BERIKUTNYA", callback_data=f"hff_next_{page}"))
        
        nav_kb = []
        if nav_row:
            nav_kb.append(nav_row)
        nav_kb.append([InlineKeyboardButton("🔍 CARI DATA", callback_data="cari_akun")])
        nav_kb.append([InlineKeyboardButton("⬅️ KEMBALI KE MENU", callback_data="back_main")])
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "data_saya_excel":
        try:
            ...
            conn.close()
            context.user_data["mode"]=None
            await update.message.reply_text(f"✅ DATA SAYA Excel masuk {saved} data!", reply_markup=main_menu_keyboard())
            return
        except Exception as e:
            await update.message.reply_text(f"❌ Gagal baca Excel DATA SAYA: {e}", reply_markup=main_menu_keyboard())
            context.user_data["mode"]=None
            return
