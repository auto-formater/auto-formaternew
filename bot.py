...
def build_display(data_dict, template, kode_pos="atas"):
    ...
    # HASIL AKUN = display_format + DATA LENGKAP section sesuai request
...
👑 LAHIR : {tgl}"""
    ...
    if data_dict.get("AKUN") and data_dict["AKUN"] != "-" and data_dict["AKUN"] != "":
        display_full += f"\n\n{data_dict['AKUN']}"
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_profil":
        ...
        # ORDER BY kode terkecil ke terbesar untuk HASIL AKUN juga
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        ...
        if not rows:
            ...
            conn.close()
            if total > 0:
                await query.edit_message_text("📑 Semua HASIL AKUN sudah terjual. Cek di HISTORY.", reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🕘 BUKA HISTORY", callback_data="menu_history")],
                    [InlineKeyboardButton("⬅️ KEMBALI", callback_data="back_main")]
                ]))
            else:
                await query.edit_message_text("Belum ada HASIL HASIL AKUN.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ KEMBALI", callback_data="back_main")]]))
            return
        ...
        kb = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("📑 2. HASIL AKUN", callback_data="history_full")],
        ...
        conn.close()
        if not rows:
            await query.edit_message_text("📑 Belum ada HASIL AKUN yang dijual.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ KEMBALI", callback_data="menu_history")]]))
            return
        await query.edit_message_text(f"📑 HISTORY HASIL AKUN - {len(rows)} terjual:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ KEMBALI", callback_data="menu_history")]]))
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode and mode.startswith("cari"):
        ...
        if "history" in mode:
            ...
            # Pencarian untuk HASIL FORMAT & HASIL AKUN yang READY
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
            kode_conds = " OR ".join(["kode LIKE ?" for _ in likes_code_patterns])
    ...
    if mode and mode.startswith("cari"):
        ...
        if "history" in mode:
            ...
            # Pencarian untuk HASIL FORMAT & HASIL AKUN - bisa cari kode, nama, KAB, dll
