...
def build_display(data_dict, template, kode_pos="atas", akun_template=None):
    # Support murni nomor urut 1-20
    full_map = {}
    for i in range(1, 21):
        full_map[str(i)] = data_dict.get(str(i), data_dict.get(f"DATA{i}", ""))
        # juga support KAB dll untuk backward compat
    # backward compat mapping
    full_map["KAB"] = full_map.get("1", "")
    full_map["KEC"] = full_map.get("2", "")
    full_map["KEL"] = full_map.get("3", "")
    full_map["SALDO"] = full_map.get("4", "")
    full_map["KELAMIN"] = full_map.get("5", "")
    full_map["KPJ"] = full_map.get("6", "")
    full_map["SENSOR"] = full_map.get("7", "")
    full_map["IT"] = full_map.get("8", "")
    full_map["PT"] = full_map.get("9", "")
    full_map["NIK"] = full_map.get("10", "")
    full_map["NIK_LENGKAP"] = full_map.get("10", "")
    full_map["KPJ_LENGKAP"] = full_map.get("11", "")
    full_map["NAMA"] = full_map.get("12", "")
    full_map["NAMA_LENGKAP"] = full_map.get("12", "")
    full_map["TGL"] = full_map.get("13", "")
    full_map["TGL_LAHIR"] = full_map.get("13", "")
    ...
    display_format = chr(10).join(result_lines)
    if not akun_template:
        akun_template = DEFAULT_AKUN_TEMPLATE
    akun_lines = []
    for line in akun_template.splitlines():
        if not line.strip():
            akun_lines.append("")
            continue
        placeholders = re.findall(r"\{(.*?)\}", line)
        if not placeholders:
            akun_lines.append(line)
            continue
        has_value = False
        rendered = line
        for ph in placeholders:
            ...
            if val and val != "-" and val.lower() != "kosong":
                has_value = True
            rendered = rendered.replace("{" + ph + "}", str(full_map.get(key, full_map.get(key.upper(), ""))))
        if has_value:
            akun_lines.append(rendered)
    akun_body = chr(10).join(akun_lines)
    if akun_body.strip():
        sep = chr(10) + chr(10) + "━━━━━━━━━━━━━━━━━━━" + chr(10) + "📋 DATA LENGKAP" + chr(10) + chr(10)
        data_lengkap_section = sep + akun_body
    else:
        data_lengkap_section = ""
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "set_template":
        ...
        if "{1}" not in new_template and "{KAB}" not in new_template and "{KAB}" not in new_template.upper():
            await update.message.reply_text(
