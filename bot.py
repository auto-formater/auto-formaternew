...
# Fungsi cek urut template nomor
def cek_template_urut(template_text):
    import re as _re
    angka = [int(n) for n in _re.findall(r'\{(\d+)\}', template_text)]
    if not angka:
        return True, "template lama (nama)"
    unik = sorted(set(angka))
    if unik[0] != 1:
        return False, f"Harus mulai dari {{1}}"
    for i in range(len(unik)-1):
        if unik[i+1] != unik[i]+1:
            return False, f"Loncat setelah {{{unik[i]}}}"
    return True, f"OK {len(unik)} baris"
...
def get_setting(user_id):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    # migrasi kolom akun_template kalau belum ada
    try:
    ...
    if not row:
        ...
        # handle berbagai panjang row lama
        if len(row) == 3:
...
def get_setting_old_compat(user_id):
    # untuk file lama yang panggil 4 values
    t, ka, kp, kpos, _, _ = get_setting(user_id)
    return t, ka, kp, kpos template, kode_atas, kode_prefix, kode_pos, base_count
...
def get_linked_users(user_id):
    """Ambil semua user_id yang terhubung dengan user_id (termasuk dirinya sendiri)"""
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    try:
...
def is_linked(user_id, target_user_id):
    if user_id == target_user_id:
        return True
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    try:
        c.execute("SELECT 1 FROM user_links WHERE (user_id_1=? AND user_id_2=?) OR (user_id_1=? AND user_id_2=?)", (user_id, target_user_id, target_user_id, user_id))
...
def boleh_lihat_grup(grup, user_id):
    ...
    try:
        ...
        allowed = [str(r[0]) for r in c.fetchall()]
        conn.close()
        return str(user_id) in allowed
    except:
        return False

def is_owner_grup(grup, user_id):
    if check_is_admin(user_id):
        return True
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("SELECT user_id FROM grup_members WHERE grup=? AND role='owner'", (grup,))
        row = c.fetchone()
        conn.close()
...
def get_grup_picker_keyboard(prefix):
    buttons = []
    for huruf in GRUP_LIST:
        buttons.append(InlineKeyboardButton(f"GRUP {huruf}", callback_data=f"{prefix}_{huruf}"))
    keyboard = []
...
def build_display(data_dict, template, kode_pos="atas", akun_template=None):
    ...
    # Pakai template akun custom jika ada
    if not akun_template:
        akun_template = DEFAULT_AKUN_TEMPLATE
    # siapkan dict untuk akun template - support banyak alias
    akun_dict = {
        "NIK": nik,
        "NIK_LENGKAP": nik,
        "KPJ": kpj_lengkap,
    ...
    try:
        ...
        # fallback kalau template pakai {1} {2} {3} {4}
        try:
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_profil":
        ...
        # BARU: jangan langsung tampilkan semua, tampilkan submenu 4 tombol
        await query.edit_message_text(
            "📄 HASIL FORMAT\n\nPilih aksi:",
            reply_markup=InlineKeyboardMarkup([
        ...
        for idx, (rid, kode, disp, kab, kec, kel, saldo, kelamin, kpj, pt) in enumerate(rows, start=1):
            ...
            kb = InlineKeyboardMarkup([[jual_btn, InlineKeyboardButton("🗑️ HAPUS", callback_data=f"del_{rid}")]])
            try:
...
Pilih aksi:",
        ...
        try:
            page = int(data.split("tampilkan_semua_format_")[1]) if "tampilkan_semua_format_" in data else 0
        except:
            page = 0
        ...
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        # FAST QUERY - pakai LIMIT OFFSET biar cepet 0.1 detik
        if is_admin_view:
            try:
                c.execute("SELECT user_id FROM admins")
                db_admins = [r[0] for r in c.fetchall()]
...
Menampilkan {len(rows)} data",
            reply_markup=InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton("⬅️ KEMBALI", callback_data="menu_hasil_format")]
            ])
        )
        
        # KIRIM PARALEL BIAR 0.1 DETIK - BUKAN SATU-SATU
        async def send_one(row_data):
            if len(row_data) == 4:
...
{disp[:3000]}", reply_markup=kb)
        ...
        # Kirim 30 sekaligus paralel -> 0.1 detik
        await asyncio.gather(*[send_one(r) for r in rows])
        
        # Nav
        nav = []
        row = []
        if page > 0:
            row.append(InlineKeyboardButton("⬅️ KEMBALI", callback_data=f"tampilkan_semua_format_{page-1}"))
        if page < total_pages - 1:
            row.append(InlineKeyboardButton("SELANJUTNYA ➡️", callback_data=f"tampilkan_semua_format_{page+1}"))
        if row:
            nav.append(row)
        ...
        start_time = time.time()
        try:
            page = int(data.split("tampilkan_semua_full_")[1]) if "tampilkan_semua_full_" in data else 0
        except:
            page = 0
        PER_PAGE = 30
        is_admin_view = check_is_admin(user_id)
...
{disp[:3000]}", reply_markup=kb)
        ...
        row = []
        if page > 0:
            row.append(InlineKeyboardButton("⬅️ KEMBALI", callback_data=f"tampilkan_semua_full_{page-1}"))
        if page < total_pages - 1:
            row.append(InlineKeyboardButton("SELANJUTNYA ➡️", callback_data=f"tampilkan_semua_full_{page+1}"))
        if row:
            nav.append(row)
        nav.append([InlineKeyboardButton("🏠 MENU", callback_data="back_main")])
        await context.bot.send_message(chat_id=user_id, text=f"✅ {len(rows)} data terkirim dalam {time.time()-start_time:.2f}s | Hal {page+1}/{total_pages}", reply_markup=InlineKeyboardMarkup(nav))
        ...
        await query.edit_message_text(
            f"📑 FORMAT+AKUN - Halaman {page+1}/{total_pages}\nTotal: {total} data | Tampil: {len(page_rows)} (30/halaman)",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔍 CARI DATA", callback_data="cari_akun")],
                [InlineKeyboardButton("⬅️ KEMBALI KE MENU", callback_data="menu_hasil_full")]
            ])
        )
        for row_data in page_rows:
            if len(row_data) == 4:
                ...
                rid, kode, disp = row_data
                owner_id = user_id
            # Cek apakah dia pemilik
            is_owner = (owner_id == user_id) or check_is_admin(user_id)
            if is_owner:
                kb = InlineKeyboardMarkup([
                    [InlineKeyboardButton("💰 JUAL", callback_data=f"konfirm_jual_{rid}"), InlineKeyboardButton("🗑️ HAPUS", callback_data=f"konfirm_hapus_{rid}")]
                ])
            else:
                # User link hanya bisa JUAL, tidak bisa HAPUS
                kb = InlineKeyboardMarkup([
                    [InlineKeyboardButton("💰 JUAL", callback_data=f"konfirm_jual_{rid}")]
                ])
            try:
                await context.bot.send_message(chat_id=user_id, text=f"{kode}\n```\n{disp[:3500]}\n```", reply_markup=kb, parse_mode="Markdown")
            except:
                await context.bot.send_message(chat_id=user_id, text=f"{kode}", reply_markup=kb)
        ...
        nav_buttons.append([InlineKeyboardButton("🔍 CARI DATA", callback_data="cari_akun")])
        nav_buttons.append([InlineKeyboardButton("🏠 MENU UTAMA", callback_data="back_main")])
        
        if nav_buttons:
            await context.bot.send_message(chat_id=user_id, text=f"📑 Halaman {page+1}/{total_pages} - {total} data", reply_markup=InlineKeyboardMarkup(nav_buttons))
        return
        ...
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        # CEK APAKAH BOLEH JUAL - ADMIN ATAU OWNER ATAU LINKED USER BOLEH
        ...
        if not row_check:
            ...
            return
        # cek apakah linked
        owner_id_check = row_check[3]
        can_sell = False
        if is_admin_seller:
            can_sell = True
        elif owner_id_check == user_id:
            can_sell = True
        elif is_linked(user_id, owner_id_check):
            can_sell = True
        
        if not can_sell:
            conn.close()
            await query.edit_message_text("⛔ Kamu tidak punya akses untuk menjual data ini.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ KEMBALI", callback_data="back_main")]]))
            return
        ...
        if is_admin_seller:
            ...
            # linked user juga boleh jual, jadi jangan filter user_id
            if is_linked(user_id, owner_id_check) or owner_id_check == user_id:
                c.execute("SELECT kode, display_full, display_format, user_id, kab, kec, kel FROM hasil_format WHERE id=?", (rid,))
            else:
        ...
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        # CEK PEMILIK - HANYA YANG BUAT FORMAT BISA HAPUS
...
💞 LAHIR : {TGL_LAHIR}"""
        ...
        await query.edit_message_text(
            ...
            "Kamu bisa atur urutan & emoji bebas.\nGunakan placeholder:\n"
            "{NIK} atau {NIK_LENGKAP}\n{KPJ} atau {KPJ_LENGKAP}\n{NAMA} atau {NAMA_LENGKAP}\n{TTL} atau {TGL_LAHIR}\n\n"
            "Atau pakai nomor urut:\n{1}=NIK, {2}=KPJ, {3}=NAMA, {4}=TTL\n\n"
            "Contoh setting:\n📩 NIK : {NIK}\n📩 KPJ : {KPJ}\n📩 NAMA : {NAMA}\n📩 TTL : {TTL}\n\n"
    ...
    elif data == "confirm_reset_akun_template":
        await query.edit_message_text(
            "⚠️ Yakin reset template akun ke default?\n\nDefault:\n" + DEFAULT_AKUN_TEMPLATE,
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("✅ SETUJU RESET", callback_data="reset_akun_template")],
                [InlineKeyboardButton("❌ BATAL", callback_data="setting_akun_template_view")]
            ])
        )
        return
        ...
        for row in rows:
            ...
            text = f"```\n{disp}\n```"
            kb = InlineKeyboardMarkup([
                [InlineKeyboardButton("🔍 CARI", callback_data=f"cari_history_{rid}"), InlineKeyboardButton("♻️ PULIHKAN", callback_data=f"pulihkan_{rid}"), InlineKeyboardButton("🗑️ HAPUS", callback_data=f"del_{rid}")]
            ])
            try:
                await context.bot.send_message(chat_id=user_id, text=text, reply_markup=kb, parse_mode="Markdown")
            except:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "data_saya_manual":
        ...
        disp_format, disp_full = build_display(parsed, template, "atas", akun_template)
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
    ...
    if mode == "admin_tambah_link_step2":
        ...
        try:
            ...
            conn.commit()
            await update.message.reply_text(
                f"✅ BERHASIL LINK!\n\n{u1} ↔ {u2}\n\nSekarang keduanya bisa saling melihat HASIL FORMAT & FORMAT+AKUN di menu:\n📄 HASIL FORMAT\n📑 FORMAT+AKUN\n\nTotal data gabungan akan tampil untuk keduanya.",
    ...
    if mode == "set_akun_template":
        ...
        if "{NIK" not in new_akun_template and "{KPJ" not in new_akun_template and "{NAMA" not in new_akun_template and "{TTL" not in new_akun_template and "{TGL" not in new_akun_template and "{1}" not in new_akun_template:
            await update.message.reply_text("❌ Template akun harus mengandung minimal {NIK} atau {1}.\nContoh: 📩 NIK : {NIK}\nCoba lagi:")
            return
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        try:
        ...
        try:
            ...
            for _, row in df.iterrows():
                try:
                    ...
                    if not parsed:
                        continue
                    disp_f, disp_full = build_display(parsed, template, "atas", akun_template)
