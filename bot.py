...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("INSERT OR IGNORE INTO users (user_id, username) VALUES (?, ?)", (user.user_id, user.username or ""))
    conn.commit()
    conn.close()
    welcome_text = (
        "🟢 MODE ON DI AKTIFKAN\n"
...
async def profil_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT COUNT(*) FROM hasil_format WHERE user_id=?", (user.user_id,))
    total = c.fetchone()[0]
    conn.close()
