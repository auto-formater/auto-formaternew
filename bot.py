...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_profil":
        ...
        kb = get_data_grup_user_keyboard()
        await query.edit_message_text("📊 DATA GRUP\nPilih grup yang mau diisi data:", reply_markup=kb)
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # === HANDLER KHUSUS GRUP STATES (TAMBAH ADMIN / ANGGOTA) ===
    if user_id in GRUP_STATES:
        state = GRUP_STATES[user_id]
        action = state.get("action")
        huruf = state.get("grup")
        text = update.message.text or ""
        # Input format saja & format+akun baru
        if action == "input_format_saja":
            data_format = text.strip()
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
            c.execute("INSERT INTO grup_data (grup, owner_id, data_format, data_akun, created_at, terjual) VALUES (?,?,?,?,?,0)",
                      (huruf, str(user_id), data_format, "", datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
            conn.commit()
            conn.close()
            await update.message.reply_text(f"✅ Data FORMAT SAJA berhasil disimpan di GRUP {huruf} dan otomatis masuk di menu ✅HASIL GRUP -> HASIL FORMAT")
            del GRUP_STATES[user_id]
            return
        if action == "input_format_akun_baru":
            if "|" in text:
                parts = text.split("|", 1)
                ...
                lines = text.split("\n")
                f_format = lines[0].strip() if lines else text
                f_akun = "\n".join(lines[1:]).strip() if len(lines)>1 else text
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
            c.execute("INSERT INTO grup_data (grup, owner_id, data_format, data_akun, created_at, terjual) VALUES (?,?,?,?,?,0)",
                      (huruf, str(user_id), f_format, f_akun, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
            conn.commit()
            conn.close()
            await update.message.reply_text(f"✅ Data FORMAT+AKUN berhasil disimpan di GRUP {huruf} dan otomatis masuk di menu HASIL GRUP -> FORMAT+AKUN")
            del GRUP_STATES[user_id]
            return
        if action == "set_admin_grup":
            ...
            for uid in ids:
                try:
                    await context.bot.send_message(chat_id=int(uid), text=f"🎉 SELAMAT! KAMU JADI ADMIN GRUP {huruf}\n━━━━━━━━━━━━━━\n👑 Kamu telah dijadikan ADMIN GRUP {huruf} oleh admin bot.\n\n✅ Sekarang kamu bisa akses:\n📊 DATA GRUP {huruf}\n✅ HASIL GRUP {huruf}\n🔐 Termasuk FORMAT+AKUN (khusus admin)\n\nSilahkan klik /start -> DATA GRUP / HASIL GRUP")
                except Exception as e:
                    print(f"Gagal notif admin {uid}: {e}")
            await update.message.reply_text(f"✅ Admin GRUP {huruf} ditambah: {', '.join(ids)}\n✅ Notifikasi sudah dikirim!", reply_markup=get_panel_admin_keyboard_mewah())
            del GRUP_STATES[user_id]
            return
        if action == "set_anggota_grup":
            ids = [x.strip() for x in text.replace(",", " ").split() if x.strip().isdigit()]
            if not ids:
                await update.message.reply_text("❌ ID tidak valid! Contoh: 111, 222, 333")
                return
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
            for uid in ids:
            ...
            for uid in ids:
                try:
                    await context.bot.send_message(chat_id=int(uid), text=f"🎉 SELAMAT! KAMU JADI ANGGOTA GRUP {huruf}\n━━━━━━━━━━━━━━\n👤 Kamu telah dijadikan ANGGOTA GRUP {huruf} oleh admin bot.\n\n✅ Sekarang kamu bisa akses:\n📊 DATA GRUP {huruf} (HANYA FORMAT & FORMAT+AKUN)\n✅ HASIL GRUP {huruf} (HASIL FORMAT)\n\nMenu DATA GRUP yang tadinya terkunci sekarang sudah terbuka!\nSilahkan klik /start -> DATA GRUP")
                except Exception as e:
                    print(f"Gagal notif anggota {uid}: {e}")
            await update.message.reply_text(f"✅ Anggota GRUP {huruf} ditambah: {', '.join(ids)}\n✅ Notifikasi sudah dikirim!", reply_markup=get_panel_admin_keyboard_mewah())
            del GRUP_STATES[user_id]
            return
        if action == "hapus_id_grup":
            ...
            conn.commit()
            conn.close()
            await update.message.reply_text(f"✅ Anggota GRUP {huruf} dihapus: {', '.join(ids)}", reply_markup=get_panel_admin_keyboard_mewah())
            del GRUP_STATES[user_id]
            return
