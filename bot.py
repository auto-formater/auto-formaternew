...
# ====== HELPER UNTUK MENU GRUP & PAKET ======
def is_paket_aktif(user_id):
    """Cek paket user. Admin selalu dianggap aktif."""
    ...
    try:
        ...
        if not row or not row[0]:
            return False
        expired_at = str(row[0]).strip()
        if expired_at.upper() == "UNLIMITED":
            return True
        try:
        ...
        print(f"⚠️ is_paket_aktif error: {e}")
        return False


def boleh_lihat_grup(huruf, user_id):
    """True jika user adalah anggota/admin grup tertentu."""
    if check_is_admin(user_id):
        return True
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("SELECT 1 FROM grup_members WHERE grup=? AND user_id=? LIMIT 1", (huruf, str(user_id)))
        ok = c.fetchone() is not None
        conn.close()
        return ok
    except Exception as e:
        print(f"⚠️ boleh_lihat_grup error: {e}")
        return False


def is_owner_grup(huruf, user_id):
    """True jika user owner/admin grup atau admin utama bot."""
    if check_is_admin(user_id):
        return True
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("SELECT role FROM grup_members WHERE grup=? AND user_id=? LIMIT 1", (huruf, str(user_id)))
        row = c.fetchone()
        conn.close()
        return bool(row and str(row[0]).lower() in ("owner", "admin"))
    except Exception as e:
        print(f"⚠️ is_owner_grup error: {e}")
        return False


def get_grup_picker_keyboard(prefix):
    """Keyboard pemilih GRUP A-O yang sesuai dengan callback handler."""
    keyboard = []
    buttons = [
        InlineKeyboardButton(f"GRUP {huruf}", callback_data=f"{prefix}_{huruf}")
        for huruf in GRUP_LIST
    ]
...
def get_linked_users(user_id):
    ...
    c = conn.cursor()
    try:
        c.execute("SELECT user_id_1, user_id_2 FROM user_links WHERE user_id_1=? OR user_id_2=?", (user_id, user_id))
        rows = c.fetchall()
        linked = set([user_id])
        for u1, u2 in rows:
            linked.add(u1)
...
def is_linked(user_id, target_id):
    if user_id == target_id:
        return True
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    try:
        c.execute("SELECT 1 FROM user_links WHERE (user_id_1=? AND user_id_2=?) OR (user_id_1=? AND user_id_2=?)", (user_id, target_id, target_id, user_id))
...
def get_next_code(user_id):
    ...
    conn.close()
    try:
        template, kode_atas, kode_prefix, kode_pos, base_count, _ = get_setting(user_id)
    except:
...
def parse_jmo_block(block_text, kode):
    # FIX: bot ngikutin baris user - preserve case, minimal 1 baris
    raw_lines = [l.strip() for l in block_text.strip().splitlines() if l.strip()!=""]
    if len(raw_lines) < 1:
        return None
    lines = raw_lines
    ...
    for i, val in enumerate(lines, start=1):
        data[str(i)] = val
        data[f"DATA{i}"] = val
    # isi field lama untuk kompatibilitas DB
    data["KAB"] = lines[0] if len(lines)>=1 else ""
    data["KEC"] = lines[1] if len(lines)>=2 else ""
    data["KEL"] = lines[2] if len(lines)>=3 else ""
    data["SALDO"] = lines[3] if len(lines)>=4 else ""
    data["KELAMIN"] = lines[4] if len(lines)>=5 else ""
    data["KPJ"] = lines[5] if len(lines)>=6 else ""
    data["SENSOR"] = lines[6] if len(lines)>=7 else ""
    data["IT"] = lines[7] if len(lines)>=8 else ""
    data["PT"] = lines[8] if len(lines)>=9 else ""
    data["NIK_LENGKAP"] = lines[9] if len(lines)>=10 else ""
    data["KPJ_LENGKAP"] = lines[10] if len(lines)>=11 else ""
    data["NAMA_LENGKAP"] = lines[11] if len(lines)>=12 else ""
    data["TGL_LAHIR"] = lines[12] if len(lines)>=13 else ""
    data["NIK"] = data.get("10","")
    data["NAMA"] = data.get("12","")
    data["KODE"] = kode
    data["AKUN"] = "\n".join(lines[13:]) if len(lines)>13 else "-"
    data["_list"] = lines
    data["_list_upper"] = lines_upper
    return data



def build_display(data_dict, template, kode_pos="atas", akun_template=None):
    # FIX: 100% ngikutin baris settingan user {1}=baris1, {2}=baris2
    import re as re_build
    full_map = {}
    for i in range(1, 51):
        full_map[str(i)] = data_dict.get(str(i), data_dict.get(f"DATA{i}", ""))
    # alias lama biar template lama tetap jalan
    full_map["KAB"] = data_dict.get("KAB", full_map.get("1",""))
    full_map["KEC"] = data_dict.get("KEC", full_map.get("2",""))
    full_map["KEL"] = data_dict.get("KEL", full_map.get("3",""))
    full_map["SALDO"] = data_dict.get("SALDO", full_map.get("4",""))
    full_map["KELAMIN"] = data_dict.get("KELAMIN", full_map.get("5",""))
    full_map["KPJ"] = data_dict.get("KPJ", full_map.get("6",""))
    full_map["SENSOR"] = data_dict.get("SENSOR", full_map.get("7",""))
    full_map["IT"] = data_dict.get("IT", full_map.get("8",""))
    full_map["PT"] = data_dict.get("PT", full_map.get("9",""))
    full_map["NIK"] = data_dict.get("NIK", full_map.get("10",""))
    full_map["NIK_LENGKAP"] = data_dict.get("NIK_LENGKAP", full_map.get("10",""))
    full_map["KPJ_LENGKAP"] = data_dict.get("KPJ_LENGKAP", full_map.get("11",""))
    full_map["NAMA"] = data_dict.get("NAMA", full_map.get("12",""))
    full_map["NAMA_LENGKAP"] = data_dict.get("NAMA_LENGKAP", full_map.get("12",""))
    full_map["TGL"] = data_dict.get("TGL_LAHIR", full_map.get("13",""))
    full_map["TGL_LAHIR"] = data_dict.get("TGL_LAHIR", full_map.get("13",""))
    ...
    def render_one(tmpl):
        ...
        for line in tmpl.splitlines():
            if line.strip() == "":
                result_lines.append("")
                continue
            placeholders = re_build.findall(r"\{(.*?)\}", line)
            if not placeholders:
            ...
            for ph in placeholders:
                key = ph.strip()
                val = str(full_map.get(key, full_map.get(key.upper(), ""))).strip()
                if val and val != "-" and val.lower() != "kosong":
                    has_value = True
                rendered_line = rendered_line.replace("{" + ph + "}", str(full_map.get(key, full_map.get(key.upper(), ""))))
            text_without_ph = re_build.sub(r"\{.*?\}", "", line).strip()
            if has_value or text_without_ph:
                result_lines.append(rendered_line)
        out = "\n".join(result_lines)
        out = re_build.sub(r"{\s*\d+\s*}", "", out)
        return out

    display_format = render_one(template)
    if not akun_template:
        akun_template = DEFAULT_AKUN_TEMPLATE
    akun_body = render_one(akun_template)
    if akun_body.strip() and "{10}" not in template:
        sep = "\n\n━━━━━━━━━━━━━━━━━━━\n📋 DATA LENGKAP\n\n"
        display_full = display_format + sep + akun_body
    else:
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_profil":
        ...
        placeholder_info = "\n\n✅ MURNI NOMOR URUT 1-20:\n{1}=Baris1 {2}=Baris2 ... {20}=Baris20\nContoh: Kab:{1} Kec:{2} Kel:{3}\nBaris kosong otomatis ke-hide"
        ...
        template, kode_atas, kode_prefix, _, _, _ = get_setting(user_id)
        next_kode = get_next_code(user_id)
        ...
        conn.close()
        if not rows:
            await query.edit_message_text("Kamu belum jadi member grup manapun.\n\nHubungi admin untuk dimasukkan ke grup.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("KEMBALI", callback_data="back_main")]]))
            return
        huruf = rows[0][0]
        is_admin_flag = check_is_admin(user_id)
        kb = get_hasil_grup_simple_keyboard(huruf, is_admin=is_admin_flag)
        await query.edit_message_text(f"✅ HASIL GRUP {huruf} - Silahkan pilih menu:", reply_markup=kb)
        return

    elif data == "panel_history_grup":
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("SELECT grup FROM grup_members WHERE user_id=?", (str(user_id),))
        rows = c.fetchall()
        ...
        huruf = rows[0][0]
        await query.edit_message_text(
            f"📚 HISTORY GRUP {huruf}",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("📜 HISTORY JUAL", callback_data=f"history_jual_grup_{huruf}")],
                [InlineKeyboardButton("⬅️ KEMBALI", callback_data="back_main")]
            ])
        )
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "data_saya_manual":
        ...
        if not parsed:
            ...
            return
        try:
            template, _, _, _, _, _ = get_setting(user_id)
        except:
    ...
    if mode == "set_template":
        ...
        # Validasi harus ada minimal {1} atau {KAB}
        if not re.search(r"{\s*\d+\s*}", new_template):
            await update.message.reply_text(
                "❌ Template harus mengandung minimal {1} atau {KAB}\n\nContoh setting pakai nomor urut:\nKab : {1}\nKec : {2}\nKel : {3}\n\nKpj : {6} ... sampai {20}\n\nKpj : {6}\n\nBaris yang kosong otomatis tidak tampil.",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ BATAL", callback_data="menu_setting")]])
            )
            return
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("UPDATE settings SET template=? WHERE user_id=?", (new_template, user_id))
        if c.rowcount == 0:
            c.execute("INSERT INTO settings (user_id, template, kode_atas, kode_prefix, kode_pos, kode_base_count, akun_template) VALUES (?,?,?,?,?,?,?)", (user_id, new_template, "001", "SEXS", "atas", 0, DEFAULT_AKUN_TEMPLATE))
        conn.commit()
        conn.close()
        context.user_data["mode"] = None
        # Preview dengan data contoh
        contoh_data = {"KAB":"DEPOK","KEC":"CILODONG","KEL":"KALIBARU","SALDO":"10.000.000","KELAMIN":"PEREMPUAN","KPJ":"2019","SENSOR":"23****","IT":"01-07-2022","PT":"INDONESIA MERDEKA","NIK":"3201234589920002","KPJ_LENGKAP":"19000378990","NAMA":"LIA","TGL":"12-12-1990"}
        preview_format, _ = build_display(contoh_data, new_template, "atas", DEFAULT_AKUN_TEMPLATE)
await update.message.reply_text(
            f"✅ Selamat template kamu sudah diubah.\n\nSilakan gunakan buat format otomatis menjadi format sesuai dengan urutan baris yang Anda setting di template ini.\n\nTemplate aktif:\n{new_template}\n\nPreview:\n{preview_format}",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⚙️ LIHAT SETTING", callback_data="setting_template_view")],[InlineKeyboardButton("⬅️ MENU", callback_data="back_main")]]),
            parse_mode="Markdown"
        )
        return
    ...
    if mode == "edit_format":
        ...
        if not parsed:
            ...
            return
        _, _, _, kode_pos, _, _ = get_setting(user_id)
        user_template, _, _, _, _, akun_template = get_setting(user_id)
    ...
    last_kode = ""
    for block in blocks:
        if len([l for l in block.splitlines() if l.strip()!=""]) < 1:
            continue
...
async def setting_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    template, kode_atas, kode_prefix, kode_pos, _, _ = get_setting(user_id)
    next_kode = get_next_code(user_id)
    pos_label = "ATAS" if kode_pos == "atas" else "BAWAH"
    await update.message.reply_text(
        f"⚙️ SETTING\nKode aktif: {next_kode} ({pos_label})",
