...
def get_next_code(user_id):
    ...
    try:
        ...
        base = 1
    # FIX: mulai dari angka yang di SET, bukan lanjut count lama
...
def main_menu_keyboard():
    # INLINE 2 BARIS di setiap menu - tambah DATA SAYA
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_profil":
        ...
        kb = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("🗑️ HAPUS SEMUA", callback_data="hapus_semua_data_saya")],
        ...
        kb_rows.append([InlineKeyboardButton("🗑️ HAPUS SEMUA", callback_data="hapus_semua_data_saya")])
    ...
    elif data.startswith("del_data_"):
        rid = int(data.split("_")[-1])
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        ...
        conn.commit()
        conn.close()
        await query.edit_message_text(f"🗑️ DATA SAYA ID {rid} dihapus.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ KEMBALI", callback_data="menu_hasil_data_saya")]]))
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode in ("set_kode_atas", "set_kode_bawah"):
        ...
        context.user_data["mode"] = None
        context.user_data["kode_pos"] = None
        kode_aktif = f"{prefix}  {int(num):07d}" if num.isdigit() else f"{prefix}  {num}"
        pos_label = "ATAS" if kode_pos=="atas" else "BAWAH"
        if kode_pos == "bawah":
