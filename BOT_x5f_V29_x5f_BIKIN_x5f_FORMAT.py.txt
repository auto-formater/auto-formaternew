...
def get_panel_admin_keyboard_mewah():
    keyboard = [
        ...
        [InlineKeyboardButton("📊 8. DATA GRUP", callback_data="panel_data"), InlineKeyboardButton("✅ 9. HASIL GRUP", callback_data="panel_hasil")],
        [InlineKeyboardButton("📚 10. HISTORY GRUP", callback_data="panel_history_grup")],
        [InlineKeyboardButton("✅ 11. AKTIFKAN GRUP", callback_data="aktifkan_grup_menu")],
        [InlineKeyboardButton("🗑️ 12. HAPUS GRUP", callback_data="hapus_grup_menu")],
...
def get_hasil_grup_simple_keyboard(huruf, is_admin=False):
    ...
    if is_admin:
        kb.append([InlineKeyboardButton("2. HASIL FORMAT+AKUN (ADMIN)", callback_data=f"hasil_akun_grup_{huruf}_0")])
    else:
        kb.append([InlineKeyboardButton("2. HASIL FORMAT+AKUN 🔒", callback_data="admin_only")])
    kb.append([InlineKeyboardButton("3. HISTORY JUAL", callback_data=f"history_jual_grup_{huruf}")])
...
def get_data_grup_simple_keyboard(huruf):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("1. HANYA FORMAT", callback_data=f"data_hanya_format_{huruf}")],
        ...
        [InlineKeyboardButton("5. KEMBALI", callback_data="back_main")]
    ])
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_profil":
        ...
        # Jika ini adalah BUAT LAGI (dipanggil setelah simpan), tampilkan pesan singkat sesuai request
        # User request: ketika tombol BUAT LAGI di klik harusnya bot membalas "SILAHKAN BUAT FORMATNYA LAGI YAH GAESS" jangan menampilkan contoh kaya awal lagi
        # Kita deteksi: jika ada last_saved atau teks sebelumnya mengandung "tersimpan", maka ini adalah BUAT LAGI
        try:
            prev_text = query.message.text or query.message.caption or ""
        except:
            prev_text = ""
        if "tersimpan" in prev_text.lower() or "format" in prev_text.lower() and "tersimpan" in prev_text.lower():
            await query.edit_message_text(
                "SILAHKAN BUAT FORMATNYA LAGI YAH GAESS",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ KEMBALI", callback_data="back_main")]]),
            )
        else:
            # Untuk pertama kali tetap tampilkan panduan, tapi untuk BUAT LAGI kita override di bawah
            await query.edit_message_text(
                "SILAHKAN BUAT FORMATNYA LAGI YAH GAESS",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ KEMBALI", callback_data="back_main")]]),
            )
        ...
        for idx, (rid, kode, disp, kab, kec, kel, saldo, kelamin, kpj, pt) in enumerate(rows, start=1):
            ...
            kb = InlineKeyboardMarkup([[jual_btn, ]])
            try:
        ...
        if row:
            ...
            # Format baru simple sesuai request user untuk GRUP juga berlaku untuk utama
            notif_text = "🔔 NOTIFIKASI JUAL\n━━━━━━━━━━━━━━\n👤 Penjual: " + str(seller_name) + " \n🆔 ID Penjual: " + str(user_id) + "\n📅 Tanggal: " + waktu_now + "\n\n" + (disp_format or disp_full)[:1000] + "\n━━━━━━━━━━━━━━"
            
            # Kirim ke owner jika beda dengan penjual
            try:
                if str(owner_id) != str(user_id):
                    await context.bot.send_message(chat_id=int(owner_id), text=notif_text)
            except:
                pass
            
            # Juga broadcast ke semua admin jika penjual admin
            if is_admin_seller:
                try:
                    c.execute("SELECT user_id FROM admins")
                    all_admins = c.fetchall()
                    for (aid,) in all_admins:
                        if str(aid) != str(user_id) and str(aid) != str(owner_id):
                            try:
                                await context.bot.send_message(chat_id=int(aid), text=notif_text)
                            except:
                                pass
                except:
                    pass
            ...
            conn.close()
            await query.edit_message_text("✅ Data " + kode + " terjual! Notifikasi terkirim.")
        else:
            await query.answer("Data tidak ditemukan", show_alert=True)
        return
        ...
        conn.close()
        if not rows:
            await query.edit_message_text("Kamu belum jadi member grup.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("KEMBALI", callback_data="back_main")]]))
            return
        huruf = rows[0][0]
        kb = get_data_grup_simple_keyboard(huruf)
        await query.edit_message_text("DATA GRUP - Silahkan pilih:", reply_markup=kb)
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in GRUP_STATES:
        ...
        if action == "input_format_saja" or action == "input_hanya_format":
            ...
            if not data_format:
                await update.message.reply_text("Data kosong, kirim lagi")
                return
            try:
                conn = sqlite3.connect(DB_PATH)
                c = conn.cursor()
                ...
                conn.commit()
                conn.close()
                await update.message.reply_text("✅ Tersimpan di GRUP " + huruf + " - " + data_format[:100] + " | Kirim lagi, 1 kirim langsung tersimpan. /start jika selesai")
            except Exception as e:
                await update.message.reply_text("Gagal: " + str(e))
            return
        if action == "input_format_akun_baru" or action == "input_format_akun":
            ...
            try:
                ...
                conn.commit()
                conn.close()
                await update.message.reply_text("✅ FORMAT+AKUN tersimpan di GRUP " + huruf + " | Kirim lagi, langsung tersimpan. /start jika selesai")
            except Exception as e:
                await update.message.reply_text("Gagal: " + str(e))
            return
        if action == "set_admin_grup":
            ...
            for uid in ids:
                try:
                    await context.bot.send_message(chat_id=int(uid), text="SELAMAT! JADI ADMIN GRUP " + huruf)
                except:
                    pass
        ...
        if action == "set_anggota_grup":
            ...
            for uid in ids:
                try:
                    await context.bot.send_message(chat_id=int(uid), text="SELAMAT! JADI ANGGOTA GRUP " + huruf)
                except:
                    pass
    ...
    for block in blocks:
        ...
        context.user_data["last_saved"] = True
        last_id = c.lastrowid
        last_kode = kode
