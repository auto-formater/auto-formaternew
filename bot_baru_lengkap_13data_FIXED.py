...
def init_db():
    ...
    try:
        ...
        pass
    c.execute("""CREATE TABLE IF NOT EXISTS hasil_format (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, kode TEXT, kab TEXT, kec TEXT, kel TEXT, saldo TEXT, kelamin TEXT, kpj TEXT, sensor TEXT, it TEXT, pt TEXT, akun TEXT, nik_lengkap TEXT DEFAULT '', kpj_lengkap TEXT DEFAULT '', nama_lengkap TEXT DEFAULT '', tanggal_lahir TEXT DEFAULT '', display_format TEXT, display_full TEXT, status TEXT DEFAULT 'ready', created_at TEXT)""")
    for col in ("nik_lengkap","kpj_lengkap","nama_lengkap","tanggal_lahir"):
        try: c.execute(f"ALTER TABLE hasil_format ADD COLUMN {col} TEXT DEFAULT ''")
        except: pass
...
def parse_jmo_block(block_text, kode):
    ...
    nik=lines[9] if len(lines)>9 else ""; kpj_lengkap=lines[10] if len(lines)>10 else ""; nama=lines[11] if len(lines)>11 else ""; tgl=lines[12] if len(lines)>12 else ""
    return {"KAB":kab,"KEC":kec,"KEL":kel,"SALDO":saldo,"KELAMIN":kelamin,"KPJ":kpj,"SENSOR":sensor,"IT":it,"PT":pt,"KODE":kode,"NIK_LENGKAP":nik,"KPJ_LENGKAP":kpj_lengkap,"NAMA_LENGKAP":nama,"TANGGAL_LAHIR":tgl,"AKUN":"-"}

def build_display(data_dict, template, kode_pos="atas"):
    kode=data_dict.get("KODE","")
    try: formatted=template.format(**data_dict)
    except: formatted=DEFAULT_TEMPLATE.format(**data_dict)
    if kode_pos=="bawah":
        body="\n".join([l for l in formatted.split("\n") if not (kode in l and len(l.strip())<30 and ("MGB" in l or "JPG" in l or re.search(r"\d{3,}",l))) ]).strip()
        display_format=f"{body}\n━━━━━━━━━━━━━━━━━━━\n     {kode}"
    else:
        lines=formatted.split("\n"); out=[]; found=False
        for l in lines:
            if not found and kode in l and len(l.strip())<30: out.append(f"     {kode}"); found=True
            else: out.append(l)
        if not found: out=[f"     {kode}","━━━━━━━━━━━━━━━━━━━"]+out
        display_format="\n".join(out).strip()
    detail="\n".join(["━━━━━━━━━━━━━━━━━━━","📋 DATA LENGKAP","━━━━━━━━━━━━━━━━━━━",f"🆔 NIK : {data_dict.get('NIK_LENGKAP') or '-'}",f"💳 KPJ LENGKAP : {data_dict.get('KPJ_LENGKAP') or '-'}",f"👤 NAMA : {data_dict.get('NAMA_LENGKAP') or '-'}",f"🎂 TANGGAL LAHIR : {data_dict.get('TANGGAL_LAHIR') or '-'}","━━━━━━━━━━━━━━━━━━━"])
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "manual" and len([l for l in text.splitlines() if l.strip()]) < 13:
        await update.message.reply_text("❌ Data belum lengkap. Untuk BUAT FORMAT MANUAL wajib 13 baris sesuai urutan.")
        return
    ...
    for block in blocks:
        ...
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("""INSERT INTO hasil_format (user_id, kode, kab, kec, kel, saldo, kelamin, kpj, sensor, it, pt, akun, nik_lengkap, kpj_lengkap, nama_lengkap, tanggal_lahir, display_format, display_full, created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                  (user_id, kode, parsed["KAB"], parsed["KEC"], parsed["KEL"], parsed["SALDO"], parsed["KELAMIN"], parsed["KPJ"], parsed["SENSOR"], parsed["IT"], parsed["PT"], parsed["AKUN"], parsed.get("NIK_LENGKAP", ""), parsed.get("KPJ_LENGKAP", ""), parsed.get("NAMA_LENGKAP", ""), parsed.get("TANGGAL_LAHIR", ""), disp_format, disp_full, datetime.now().isoformat()))
