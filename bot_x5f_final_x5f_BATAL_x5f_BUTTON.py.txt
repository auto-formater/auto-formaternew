...
def get_next_sex_code(user_id):
    ...
    max_code = 0
    try:
        c.execute("SELECT sex_code FROM hasil_format WHERE user_id=?", (user_id,))
        rows = c.fetchall()
        for (sc,) in rows:
            if not sc:
                continue
        ...
        pass
    try:
        c.execute("SELECT sex_code FROM riwayat WHERE user_id=?", (user_id,))
        rows = c.fetchall()
        for (sc,) in rows:
            if not sc:
                continue
...
def parse_jmo_block(text_block, user_id, sex_counter=1):
    ...
    cleaned = []
    for l in clean:
        if ":" in l and any(k in l.upper() for k in ["KAB", "KEC", "KEL", "SALDO", "KELAMIN", "KPJ", "SENSOR", "IT", "PT"]):
            parts = l.split(":",1)
    ...
    if len(cleaned) < 9:
        return None, f"Butuh 9 baris, kamu kirim {len(cleaned)}"
    kab, kec, kel, saldo, kelamin_raw, kpj, sensor, iuran_t, pt = cleaned[:9]
    raw_format_lines = cleaned[:9]
    raw_akun_lines = cleaned[9:] if len(cleaned) > 9 else []
    kelamin = kelamin_raw.upper().replace("PEREMPUAN","P").replace("LAKI-LAKI","L").replace("LAKI","L").replace("WANITA","P").strip()
    ...
    if raw_akun_lines:
        for l in raw_akun_lines:
            if re.match(r"^\d{16}$", l.replace(" ","")):
                kode = l; break
        if not kode:
            kode = raw_akun_lines[0] if raw_akun_lines else ""
...
def process_blocksdef process_blocks(full_text, user_id):
    blocks = re.split(r'\n\s*\n', full_text.strip())
    if len(blocks)==1:
        lines=[l for l in full_text.strip().splitlines() if l.strip()!=""]
        if len(lines)>=9:
            if len(lines)%9==0:
                blocks=["\n".join(lines[i:i+9]) for i in range(0,len(lines),9)]
            elif len(lines)%13==0:
...
def four_buttons_format(id_data, display_text=""):
    # COPY langsung ke clipboard OTOMATIS - tidak ada BUAT LAGI / KEMBALI di per kotak
    if HAS_COPY and display_text:
...
def bottom_menu_after_hasil():
    # Menu BUAT LAGI dan KEMBALI hanya di akhir hasil, 1x saja
    kb = [
        [InlineKeyboardButton("🔄 BUAT LAGI", callback_data="bf_manual"), InlineKeyboardButton("⬅️ KEMBALI MENU UTAMA", callback_data="main")],
    ]
    return InlineKeyboardMarkup(kb)
...
👋 Selamat datang, {first_name}! Sukses selalu, tetap semangat dan jangan lupa bersyukur untuk hari ini. Silahkan pilih menu di bawah ini : 👇"""
    ...
    if data == "bikin_format":
        ...
        await query.edit_message_text("⌨️ KIRIM DATA MANUAL - 9 baris format
/cancel untuk batal", reply_markup=InlineKeyboardMarkup([
    ...
    if data == "bf_excel":
        ...
        await query.edit_message_text("📤 Upload file .xlsx\n/cancel untuk batal", reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("❌ BATAL", callback_data="cancel_batal"), InlineKeyboardButton("🔙 KEMBALI MENU UTAMA", callback_data="main")]
        ]))
        return
    ...
    if data == "hasil_format":
        ...
        if data == "hf_utuh":
        # HASIL FORMAT UTUH = cuma FORMAT saja, tanpa AKUN, tanpa ID KODE
        conn=sqlite3.connect(DB_PATH)
        c=conn.cursor()
        ...
        for rid, display, kode, sex in rows:
            ...
            await context.bot.send_message(chat_id=query.message.chat_id, text=f"{display}"[:4000], reply_markup=kb)
        return

        if data == "hf_kode":
        # HASIL KODE = cuma list kode saja dalam 1 kotak, OTOMATIS
        conn=sqlite3.connect(DB_PATH)
        c=conn.cursor()
        ...
        if not rows:
            ...
            return
        # Gabung semua kode dalam 1 kotak seperti request: SEX 001, SEX 002...
        kode_list = []
        for (sc,) in rows:
            if not sc:
                continue
            # Pastikan ada prefix SEX
            sc_str = str(sc).strip()
            if sc_str.upper().startswith("SEX"):
                kode_list.append(sc_str)
            else:
                kode_list.append(f"SEX {sc_str}")
        combined_kode = "
".join(kode_list)
        # Tampilkan dalam 1 kotak saja
        ...
        # COPY clipboard untuk semua kode
        if HAS_COPY:
            kb_kode = InlineKeyboardMarkup([[InlineKeyboardButton("📋 COPY SEMUA KODE", copy_text=CopyTextButton(combined_kode[:4000]))],
                                            [InlineKeyboardButton("🔙 KEMBALI MENU UTAMA", callback_data="main")]])
        else:
        ...
        for rid, kode, display, sex in rows:
            kb = four_buttons_format(rid, display if 'display' in locals() else "")
            await context.bot.send_message(chat_id=query.message.chat_id, text=f"SEX {sex}"[:4000], reply_markup=kb)
        return
    ...
    if data == "hasil_akun":
        ...
        if data == "ha_utuh":
        # HASIL AKUN UTUH sekarang = FORMAT + AKUN gabung OTOMATIS
        conn=sqlite3.connect(DB_PATH)
        c=conn.cursor()
        # Ambil dari hasil_format biar FORMAT + AKUN bisa gabung
        ...
        for rid, display, akun, kode, sex in rows:
            # Gabung FORMAT + AKUN sesuai request
...
{akun}"
                ...
                combined = f"{display}"
            # COPY yang bisa langsung copy ke clipboard
            kb = four_buttons_format(rid, combined) if 'four_buttons_format' in globals() else four_buttons_akun(rid)
            await context.bot.send_message(chat_id=query.message.chat_id, text=combined[:4000], reply_markup=kb)
        # Menu bawah cuma 1x
        ...
        for rid, content, kode in rows:
            ...
            await context.bot.send_message(chat_id=query.message.chat_id, text=f"{content}"[:4000], reply_markup=kb)
        return

        if data == "ha_kode":
        # HASIL AKUN KODE = SEX 003 + garis + AKUN saja (tanpa FORMAT), OTOMATIS
        conn=sqlite3.connect(DB_PATH)
        c=conn.cursor()
        # Ambil dari hasil_format biar bisa dapet SEX + AKUN
        ...
        for rid, sex, akun, kode in rows:
            # Format sesuai request: SEX 003 + garis + akun
            if not sex:
                sex = kode or "000"
...
{akun}"
                ...
                # kalau akun kosong, tetap tampil SEX + garis
                combined = f"{sex_str}
━━━━━━━━━━━━━━━
{kode or '-'}"
    ...
    if data.startswith("dup_format_"):
        ...
        kb = four_buttons_format(new_id, new_display if 'new_display' in locals() else "")
        await context.bot.send_message(chat_id=query.message.chat_id, text=f"{new_display}"[:4000], reply_markup=kb)
        return
    ...
    if data == "set_templat":
        ...
        setting = get_setting(user_id)
        tmpl = setting.get("template", DEFAULT_TEMPLATE)
        kode_next = f"{get_next_sex_code(user_id):03d}"
        # Format template preview jadi lebih rapi
        tmpl_preview = tmpl.replace("{KAB}", "{KAB}").replace("{KEC}", "{KEC}")
        # Tampilkan sesuai request user
        text_setting = f"""⚙️ SETTING AUTO FORMAT
...
---
Template saat ini (raw):
{tmpl}

Kamu bisa memakai placeholder:
{{KAB}} {{KEC}} {{KEL}} {{SALDO}} {{KELAMIN}} {{KPJ}} {{SENSOR}} {{IT}} {{PT}} {{KODE_ATAS}}

Kirim template baru dengan placeholder di atas.
/cancel untuk batal"""
        await query.edit_message_text(text_setting, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 KEMBALI MENU UTAMA", callback_data="main")]]))
        return

    if data == "set_kode_atas":
        context.user_data["state"]="awaiting_kode_atas"
        await query.edit_message_text("""🔢 SET KODE ATAS
___________________
...
Ketik kode sekarang:
/cancel untuk batal""", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 KEMBALI MENU UTAMA", callback_data="main")]]))
        return

    if data == "set_kode_bawah":
        context.user_data["state"]="awaiting_kode_bawah"
        await query.edit_message_text("""🔢 SET KODE BAWAH
___________________
...
async def message_handler(update, context):
    ...
    if state=="awaiting_search_akun":
        ...
        if not found:
            ...
            for rid, content, kode in found[:10]:
                kb = four_buttons_akun(rid)
                await context.bot.send_message(chat_id=update.effective_chat.id, text=f"{content}"[:4000], reply_markup=kb)
        context.user_data["state"]=None
        return
    ...
    if state=="awaiting_search_all_admin":
        ...
        if not found:
            ...
            for rid, display, kode, sex, uid in found[:10]:
                kb = four_buttons_format(rid, display if 'display' in locals() else "")
                await context.bot.send_message(chat_id=update.effective_chat.id, text=f"User:{uid} SEX:{sex} {display}"[:4000], reply_markup=kb)
        context.user_data["state"]=None
        return
    ...
    if state=="awaiting_edit_format":
        ...
        await context.bot.send_message(chat_id=update.effective_chat.id, text=f"{p['content_display']}"[:4000], reply_markup=kb)
        return
    ...
    if state=="awaiting_manual":
        ...
        for nid in new_ids:
            ...
            if row:
                ...
                await context.bot.send_message(chat_id=update.effective_chat.id, text=f"{display}"[:4000], reply_markup=kb)
        conn.close()
        return
...
async def excel_handler(update, context):
    ...
    try:
        ...
        c=conn.cursor()
        for nid in new_ids:
            c.execute("SELECT content_display FROM hasil_format WHERE id=?", (nid,))
            row=c.fetchone()
            if row:
                display = row[0]
        ...
        # bottom menu
        try:
            await context.bot.send_message(chat_id=update.effective_chat.id, text="✅ Selesai - Mau bikin lagi?", reply_markup=bottom_menu_after_hasil())
        except:
            pass
        conn.close()
